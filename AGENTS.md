...
`docs/` is a **third** npm package (`"name": "site"`), and unlike the other two it is outside all five gates: the root `tsconfig.json` names it in `exclude`, `docs/package.json` declares neither `typecheck` nor `lint`, it has no `eslint.config.mts` of its own, and while `@astrojs/check` is a dependency nothing ever runs `astro check`. So the package that produces the public documentation site is the one with no gate at all, and a standalone `npx tsc --noEmit` inside it is red today. The "both" above means the two that are checked, not the whole branch: `docs/scripts/helpers/` holds its own hand-copies of the `scripts/helpers/` tooling, and nothing reads them.
