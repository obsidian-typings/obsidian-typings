...
`main` carries no build — it is the docs and tooling branch — so its whole gate is five npm scripts: `lint`, `lint:md`, `format:check`, `spellcheck` and `typecheck`. **No workflow in `.github/workflows` runs any of them.** They fire from the pre-commit hook (`nano-staged` → `lint:fix` + `format` on staged `.ts`/`.mts`, `lint:fix` alone on `.astro`, `spellcheck` on everything staged, `lint:md:fix` on `.md`) and from a human running them, and that is the only thing standing between a broken release script and a release. The `.astro` entry has no `format` beside it because dprint loads a TypeScript, a JSON and a markdown plugin and none of them formats Astro; it was added with `eslint-plugin-astro` in 2026-09, since before that a staged `.astro` change matched no task but `spellcheck`.

`npm run typecheck` is `tsc --noEmit`, and it exists in **both** of the checked packages because it has to be run in both. The root `tsconfig.json` does `include` `workflow-scripts/**/*.ts`, but it checks those files against the root's dependencies, `lib` and `target` rather than the subpackage's own — a near-miss, not a substitute for `npm run typecheck` inside `workflow-scripts`.
...
`docs/` is a **third** npm package (`"name": "site"`), and unlike the other two it is outside the **type** gate specifically: the root `tsconfig.json` names it in `exclude`, `docs/package.json` declares neither `typecheck` nor `lint`, and while `@astrojs/check` is a dependency nothing ever runs `astro check`. So a standalone `npx tsc --noEmit` inside it is red today and no gate reports that. The "both" above means the two packages that are *type*-checked, not the whole branch.

**It is NOT outside the other four**, and the reason is the rule in the section below: `docs/` has no `eslint.config.mts` of its own, so the **nearest** config to its files is the root one — which names `docs/**/*.ts` in `typeScriptFiles` and carries three dedicated `getDocsOverrideConfigs()` blocks that would be pointless otherwise. `spellcheck` runs `cspell` over `.`, and `dprint` excludes only lock files and `**/*.md`, both reaching `docs/` the same way. Measured 2026-09-15, when tightening the root ESLint config put four new findings in `docs/scripts/helpers/` and `npm run lint` at the root failed on them. Do not read "no `eslint.config.mts` of its own" as "not gated" — for ESLint it means the opposite.

`docs/scripts/helpers/` holds its own hand-copies of the `scripts/helpers/` tooling.
