...
`scripts/helpers/eslint/` and `workflow-scripts/helpers/eslint/` each hold hand-copied sources of rules whose upstream is `obsidian-dev-utils`, at `src/script-utils/linters/eslint-rules/`. The root tree registers all three — `no-unused-params-members`, `no-used-underscore-variables`, `readonly-params-options-result-members` — and `workflow-scripts` registers the last two, forked for the same self-contained-island reason `exec.ts` is forked above. *Which* rules a tree registers is per-consumer by construction and is not drift; what each vendored source *contains* is.

**A source is meant to differ from upstream by exactly one line: the `type-guards.ts` import**, which upstream reaches at `../../../type-guards.ts` and both trees here reach at `../type-guards.ts`. So a sync is a plain copy plus that one substitution, and every other difference is drift. `workflow-scripts/helpers/type-guards.ts` exists only to supply `assertNonNullable` and `ensureNonNullable` to its two copies; the root tree's `scripts/helpers/type-guards.ts` already exported both.
...
**The folder and plugin names diverge from upstream's, and that is unresolved rather than settled.** The directory is `eslint/` and the registry is `local-plugin.ts` exporting `localPlugin` under a `local/` config namespace, where upstream's other consumers use `eslint-rules/`, `obsidian-dev-utils-plugin.ts` and an `obsidian-dev-utils/` namespace. Converging is a rename that has to land on all three tracked branches at once, and is bound up with a question this repo has not answered — it has no test project on any branch, so the test file upstream ships beside each rule source cannot travel with it, and none of them is vendored here. Renaming the underscore rule was not part of that: its name is forced by its behavior, and leaving a rule called `…-params` reporting locals would have been a worse lie than the divergence.
...
**The write side has an open question of its own, and npm's own documentation looks like it answers it.** `npm trust github` *creates* a configuration rather than reconciling one — `createConfig` in `npm/lib/trust-cmd.js` POSTs `[trustConfig]` and reads nothing beforehand — so what a second, identical call does to a package that already has a publisher is the registry's business alone, and nobody here has made one. npm's bundled `npm-trust.md` says the registry "only supports one configuration per package" and that creating a second "will result in an error", which would make a blind call harmless. **That paragraph is stale**: it was written on 2026-02-11 and last touched on 2026-06-03, while npmjs.com's trusted-publishers page documented on 2026-09-03 that a package may carry **up to ten** publishers, added and deleted independently. A registry that accepts ten is not one that refuses the second, so `resolvePublisher`'s `unknown` arm still asks the operator instead of attaching blind. Settling it needs a write on that endpoint, and every write there costs a one-time password — the reason it is open rather than merely untried.
