...
const typingsVersion = (JSON.parse(readFileSync(TYPINGS_JSON_PATH, 'utf-8')) as PackageJsonPartial).version;

const OBSIDIAN_VERSION_RESOLUTION: Record<string, string> = {
...
type ReflectionWithSources = DeclarationReflection | SignatureReflection;

export function load(app: Application): void {
  const packages = getPackages();

  app.converter.on(Converter.EVENT_RESOLVE_BEGIN, (context) => {
    const namespaces = context.project.getReflectionsByKind(ReflectionKind.Namespace);
    for (const ns of namespaces) {
      const decl = ns as DeclarationReflection;
      const sourceTag = decl.comment?.blockTags.find((tag) => tag.tag === '@source');
      if (sourceTag) {
        const content = sourceTag.content[0];
        if (!content) {
          continue;
        }
        ...
        setAllSources(decl, packages, lastPart, source, offset);
        traverseChildren(decl, (child) => {
          setAllSources(child, packages, lastPart, source, offset);
        });
      } else {
        setAllSources(decl, packages);
        traverseChildren(decl, (child) => {
          setAllSources(child, packages);
        });
      }
    }
  });
...
function getPackages(): Partial<Record<string, PackageInfo>> {
  return {
    obsidian: {
      github: 'obsidianmd/obsidian-api',
      version: OBSIDIAN_VERSION_RESOLUTION[readPackageVersion(`${ROOT_FOLDER}node_modules/obsidian/package.json`)]
    },
    typescript: {
      github: 'microsoft/TypeScript',
      version: readPackageVersion(`${ROOT_FOLDER}node_modules/typescript/package.json`),
      versionPrefix: 'v'
    }
  };
}
...
function readPackageVersion(path: string): string {
  return (JSON.parse(readFileSync(path, 'utf-8')) as PackageJsonPartial).version;
}
...
function setSources(ref: ReflectionWithSources, packages: Partial<Record<string, PackageInfo>>, fileName = '', filePath = '', offset = 0): void {
  if (!ref.sources || ref.sources.length === 0) {
    return;
  }

  for (const source of ref.sources) {
    updateSource(source, packages, fileName, filePath, offset);
  }
}

function traverseChildren(ref: DeclarationReflection, cb: (child: DeclarationReflection) => void): void {
  if (ref.children) {
...
function updateSource(source: SourceReference, packages: Partial<Record<string, PackageInfo>>, fileName: string, filePath: string, offset: number): void {
  if (source.fullFileName.includes('node_modules') || (filePath.includes('node_modules') && !source.url)) {
    const path = (filePath || source.fullFileName).split('node_modules/')[1];
    if (!path) {
      return;
    }
    ...
    source.fileName = path.split('/').pop() ?? source.fileName;
    source.fullFileName = path.slice(packageName.length + 1);
    const version = pkg.version ? `${pkg.versionPrefix ?? ''}${pkg.version}` : 'HEAD';
    source.url = `https://github.com/${pkg.github}/blob/${version}/${source.fullFileName}#L${String(source.line - offset)}`;
    source.line -= offset;
  } else if (source.fullFileName.includes(ENTRYPOINT_NAME) && filePath) {
    source.fileName = fileName;
    source.fullFileName = filePath;
    source.line -= offset;
    source.url = `https://github.com/${filePath}#L${String(source.line)}`;
  } else if (source.url?.match(/blob\/[0-9a-f]{5,40}\/src/)) {
    source.url = source.url.replace(/blob\/[0-9a-f]{5,40}\/src/, `blob/${typingsVersion}/src`);
  }
}
